...
data_clean <- read_csv("~/Desktop/NFS 2026/NFS1218/Data Analysis Pipeline/clean_precision_growth_dataset_2variables.csv")
...
fviz_nbclust(df, kmeans, method = "wss") +
  geom_vline(xintercept = 6, linetype = 2) #x-intercept from 4 to 6
...
fviz_cluster(
  km.res,
  data = df,
  palette = c("#2E9FDF", "#00AFBB", "#E7B800", "#FC4E07", "#ff69b4","#000000"),
  ellipse.type = "euclid",
...
#we have 5 instead of 6 here!!!!!

# Compute PAM with k = 5
pam.res <- pam(df, 5)
# Print the results
print(pam.res)
...
dd <- cbind(data_clean, cluster = pam.res$cluster)
head(dd, n = 3)
...
fviz_cluster(
  pam.res,
  palette = c("#00AFBB", "#FC4E07", "#000000", "#ff69b4","#E7B800"),
  # color palette
  ellipse.type = "t",
  # Concentration ellipse
  repel = TRUE,
...
fviz_dend(
  ...
  cex = 0.5,
  # label size
  k_colors = c("#2E9FDF", "#00AFBB", "#E7B800", "#FC4E07", "#000000", "#ff69b4"),
